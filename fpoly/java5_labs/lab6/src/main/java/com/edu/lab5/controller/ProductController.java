...
public class ProductController {
    ...
    public String searchAndPage1(ModelMap model, @RequestParam("keywords") Optional<String> kw,
        ...
        Pageable pageable = PageRequest.of(page.orElse(0), 5);
        Page<Product> pages = dao.findAllThings(kwords, pageable); 
        // try {
        //     Integer numberKwords = Integer.parseInt(kwords);
        //     pages = dao.findAllByIdIs(numberKwords, pageable);
        // } catch (Exception e) {
        //     try {
        //         Double dPrice = Double.parseDouble(kwords);
        //         pages = dao.findAllByPriceLike("%" + dPrice + "%", pageable);
        //     } catch (Exception e2) {
